...
### For ease of use all mappings are done with EasyDicts
### Arguments whose values are tuples will be added to the CLI
### argparser with the first value as default and the second as
### the arg keyword arguments.
### You can define non-CLI arguments by not using iterables as values
### e.g: __C.param_not_in_cli = 'param_not_in_cli'
### Params not in CLI still can be modified trough a config file
...
__C.test = (False, edict(help="run testing only", action="store_true"))
